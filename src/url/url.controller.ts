...
export class UrlController {
  ...
  @ApiResponse({ status: 201, description: 'URL encurtada com sucesso.', type: String })
  @ApiResponse({ status: 400, description: 'Formato de URL inválido.' })
  @ApiResponse({ status: 409, description: 'Não foi possível gerar um código único.' })
  ...
  @ApiParam({ name: 'shortCode', description: 'O código curto da URL', example: 'aZbKq7' })
  @ApiResponse({ status: 301, description: 'Redirecionamento bem-sucedido para a URL original.' })
  @ApiResponse({ status: 404, description: 'URL curta não encontrada ou excluída.' })
  async redirect(@Param('shortCode') shortCode: string, @Res() res: Response) {
    try {
      ...
      res.status(HttpStatus.NOT_FOUND).json({ message: error.message || 'URL não encontrada.' });
    }
  }
}