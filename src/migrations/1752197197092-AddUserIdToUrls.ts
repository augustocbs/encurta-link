...
export class AddUserIdToUrls1752197197092 implements MigrationInterface {
  name = 'AddUserIdToUrls1752197197092';

  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `CREATE TABLE \`urls\` (\`id\` int NOT NULL AUTO_INCREMENT, \`userId\` int NULL, \`originalUrl\` varchar(767) NOT NULL, \`shortCode\` varchar(6) NOT NULL, \`clicks\` int NOT NULL DEFAULT '0', \`createdAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updatedAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`deletedAt\` datetime(6) NULL, UNIQUE INDEX \`IDX_aacdba4c65e535a14ccf5db306\` (\`originalUrl\`), UNIQUE INDEX \`IDX_34ced802e4a45bf6a6346f2eb9\` (\`shortCode\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `DROP INDEX \`IDX_aacdba4c65e535a14ccf5db306\` ON \`urls\``,
    );
